...
class PerssonModelGUI_V2:
    ...
    def _create_equations_tab(self, parent):
        ...
        # ═══════════════════════════════════════════════════════
        # Section 3: Flash Temperature — Per-q Sequential Accumulation
        # ═══════════════════════════════════════════════════════
        add_separator()
    ...
    def _create_flash_temp_tab(self, parent):
        ...
        ttk.Label(enable_frame,
                  text="활성화 시 파수별(per-q) 열 누적 방식으로\n"
                       "flash temperature를 고려합니다.\n"
                       "Cold pass → q₀→q₁ 순차 ΔT 누적 (WLF feedback)",
        ...
        # ===== 2-C. Self-Consistent Iteration 파라미터 =====
        sc_frame = self._create_section(left_panel, "2-C) Self-Consistent 반복 설정")

        sc_row1 = ttk.Frame(sc_frame)
        sc_row1.pack(fill=tk.X, pady=1)
        ttk.Label(sc_row1, text="최대 반복:", font=self.FONTS['body']).pack(side=tk.LEFT)
        self.flash_max_iter_var = tk.StringVar(value="8")
        ttk.Entry(sc_row1, textvariable=self.flash_max_iter_var, width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(sc_row1, text="회", font=self.FONTS['small']).pack(side=tk.LEFT)

        sc_row2 = ttk.Frame(sc_frame)
        sc_row2.pack(fill=tk.X, pady=1)
        ttk.Label(sc_row2, text="수렴 허용 오차:", font=self.FONTS['body']).pack(side=tk.LEFT)
        self.flash_tol_var = tk.StringVar(value="0.5")
        ttk.Entry(sc_row2, textvariable=self.flash_tol_var, width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(sc_row2, text="°C", font=self.FONTS['small']).pack(side=tk.LEFT)

        sc_row3 = ttk.Frame(sc_frame)
        sc_row3.pack(fill=tk.X, pady=1)
        ttk.Label(sc_row3, text="이완 계수:", font=self.FONTS['body']).pack(side=tk.LEFT)
        self.flash_relax_var = tk.StringVar(value="0.5")
        ttk.Entry(sc_row3, textvariable=self.flash_relax_var, width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(sc_row3, text="(0~1, Under-relaxation)", font=self.FONTS['small']).pack(side=tk.LEFT)

        ttk.Label(sc_frame,
                  text="ΔT 수렴까지 전체 파수 적분을 반복합니다.\n"
                       "이완 계수 α: ΔT_next = α·ΔT_old + (1-α)·ΔT_new\n"
                       "α=0.5 권장 (진동 방지)",
                  font=self.FONTS['small'], foreground='#64748B').pack(anchor=tk.W, pady=2)
        ...
        ttk.Label(formula_frame,
                  text="[Self-Consistent Iteration 순서]\n"
                       "1. Cold pass: G(q), P(q), μ_cold 계산\n"
                       "2. 초기 ΔT 추정 (μ_cold, P_macro_cold)\n"
                       "3. REPEAT until 수렴:\n"
                       "   → T_hot = T_base + ΔT\n"
                       "   → WLF shift → aT(T_hot)\n"
                       "   → E'(ω·aT), E''(ω·aT) 추출\n"
                       "   → G_hot(q), P_hot(q) 재계산\n"
                       "   → μ_hot, A/A0_hot 산출\n"
                       "   → P_macro_hot 캡처 (q_macro)\n"
                       "   → ΔT_new = f(μ_hot, P_macro_hot)\n"
                       "   → 수렴? → 탈출\n"
                       "   → ΔT = α·ΔT_old + (1-α)·ΔT_new",
        ...
        # Create figure with 3x2 subplots for flash temperature results
        self.fig_flash_temp = Figure(figsize=(9, 10), dpi=100)
        ...
        self.fig_flash_temp.subplots_adjust(left=0.12, right=0.92, top=0.97, bottom=0.06, hspace=0.55, wspace=0.38)
    ...
    def _update_flash_temp_plots(self, v, mu_cold, mu_hot, A_A0_cold, A_A0_hot, flash_results):
        ...
        try:
            ...
            delta_T_profile = flash_results.get('delta_T_profile')  # 2D: (n_q, n_v)
            q_array = flash_results.get('q_array')

            # === Row 1 Left: ΔT vs velocity ===
            ...
            self.ax_flash_Thot.semilogx(v, flash_results['T_hot'], 'r-', linewidth=2.5,
            ...
            self.ax_flash_dT_q.clear()
            if delta_T_profile is not None and q_array is not None:
                n_v_total = delta_T_profile.shape[1]
                # Select representative velocities (up to 8 curves)
                n_curves = min(8, n_v_total)
                if n_v_total <= n_curves:
                    v_indices = list(range(n_v_total))
                else:
            ...
            if hasattr(self, '_flash_heatmap_cb') and self._flash_heatmap_cb is not None:
                try:
                    self._flash_heatmap_cb.remove()
                except Exception:
                    pass
                self._flash_heatmap_cb = None
            self.ax_flash_heatmap.clear()
            if delta_T_profile is not None and q_array is not None and len(v) > 1:
                log_v = np.log10(np.maximum(v, 1e-20))
                log_q = np.log10(np.maximum(q_array, 1e-20))

                # delta_T_profile is (n_q, n_v) → plot with v on x, q on y
                dT_max = np.max(delta_T_profile)
                if dT_max > 0:
                    pcm = self.ax_flash_heatmap.pcolormesh(
                        log_v, log_q, delta_T_profile,
            ...
            # 1열: log10(ζ) = log10(q/q0), 2열: T(ζ) = T_base + ΔT_accumulated [°C]
            _preset_zeta_dT = [
                ...
                (4.56e-01, 2.01e+01), (4.69e-01, 2.01e+01), (4.82e-01, 2.01e+01),
                (4.95e-01, 2.01e+01), (5.08e-01, 2.01e+01), (5.21e-01, 2.01e+01),
                (5.34e-01, 2.01e+01), (5.47e-01, 2.01e+01), (5.60e-01, 2.01e+01),
                (5.73e-01, 2.01e+01), (5.86e-01, 2.01e+01), (5.99e-01, 2.01e+01),
                (6.12e-01, 2.01e+01), (6.25e-01, 2.01e+01), (6.38e-01, 2.01e+01),
                (6.51e-01, 2.01e+01), (6.64e-01, 2.01e+01), (6.77e-01, 2.01e+01),
                (6.90e-01, 2.01e+01), (7.03e-01, 2.01e+01), (7.16e-01, 2.01e+01),
                (7.30e-01, 2.01e+01), (7.43e-01, 2.01e+01), (7.56e-01, 2.01e+01),
                (7.69e-01, 2.01e+01), (7.82e-01, 2.01e+01), (7.95e-01, 2.01e+01),
                (8.08e-01, 2.01e+01), (8.21e-01, 2.01e+01), (8.34e-01, 2.01e+01),
                (8.47e-01, 2.01e+01), (8.60e-01, 2.01e+01), (8.73e-01, 2.01e+01),
                (8.86e-01, 2.01e+01), (8.99e-01, 2.01e+01), (9.12e-01, 2.01e+01),
                (9.25e-01, 2.01e+01), (9.38e-01, 2.01e+01), (9.51e-01, 2.01e+01),
                (9.64e-01, 2.01e+01), (9.77e-01, 2.01e+01), (9.90e-01, 2.02e+01),
                (1.00e+00, 2.02e+01), (1.02e+00, 2.02e+01), (1.03e+00, 2.02e+01),
                (1.04e+00, 2.02e+01), (1.06e+00, 2.02e+01), (1.07e+00, 2.02e+01),
                (3.54e+00, 2.55e+01), (3.56e+00, 2.55e+01), (3.57e+00, 2.55e+01),
                (3.58e+00, 2.55e+01), (3.60e+00, 2.56e+01), (3.61e+00, 2.56e+01),
            ...
            if A_A0_cold is not None:
                self.ax_flash_area_compare.semilogx(v, A_A0_cold, 'b-', linewidth=2, marker='o',
                                                     markersize=3, label='A/A0 Cold')
            ...
            flash_model_disp = flash_results.get('flash_model', 'greenwood')
            model_label = "Greenwood SC 반복" if flash_model_disp == "greenwood" else "Persson Full Integral"
            self.flash_result_text.insert(tk.END, f"Flash Temperature 결과 ({model_label})\n")
            self.flash_result_text.insert(tk.END, "=" * 50 + "\n")
            self.flash_result_text.insert(tk.END, f"T_base = {T_base:.1f} °C\n")
            self.flash_result_text.insert(tk.END,
                f"ΔT 범위 (per-q accum): {np.min(flash_results['delta_T']):.1f} ~ "
            ...
            if delta_T_cold is not None:
                ...
                if np.max(delta_T_cold) > 0.1:
                    ...
                    self.flash_result_text.insert(tk.END, f"ΔT 감소율 (WLF 피드백): {reduction_dT:+.1f}%\n")

            self.flash_result_text.insert(tk.END,
                f"\nT_hot 범위: {np.min(flash_results['T_hot']):.1f} ~ "
                f"{np.max(flash_results['T_hot']):.1f} °C\n")
            self.flash_result_text.insert(tk.END,
                f"Jd 범위: {np.min(flash_results['Jd']):.2e} ~ {np.max(flash_results['Jd']):.2e}\n")
            self.flash_result_text.insert(tk.END,
                f"q_dot 범위: {np.min(flash_results['q_dot']):.2e} ~ "
                f"{np.max(flash_results['q_dot']):.2e} W/m²\n\n")
            ...
            if delta_T_profile is not None and q_array is not None:
                ...
                if iters is not None and len(iters) > 0:
                    self.flash_result_text.insert(tk.END, f"\n[Per-q Temperature Accumulation 정보]\n")
                    self.flash_result_text.insert(tk.END,
                        f"  방식: Per-q Accumulation (Persson 2006)\n")
                    self.flash_result_text.insert(tk.END,
                        f"  각 파수 q_i에서 WLF shift가 누적 온도 기반으로 적용됨\n")
                self.flash_result_text.insert(tk.END,
                    f"  파수 범위: {q_array[0]:.2e} ~ {q_array[-1]:.2e} 1/m ({len(q_array)} pts)\n")
                self.flash_result_text.insert(tk.END,
                    f"  열유속: q̇ = μ·σ₀·v / P(q)  (A/A₀ 보정 적용)\n")
            ...
            self.flash_result_text.insert(tk.END,
                ...
                f"{'μ_cold':>8} {'μ_hot':>8}\n")
            self.flash_result_text.insert(tk.END, "-" * 62 + "\n")
            step = max(1, len(v) // 15)
            ...
            try:
                ...
                def get_header():
                    return [
                        ...
                        f"# Flash 방식,Self-Consistent Iteration (Persson 2006)",
                        "#"
                    ]
    ...
    def _export_flash_zeta_dT_csv(self):
        """Export per-magnification ΔT data (ζ vs T) for selected velocity."""
        if not hasattr(self, 'mu_visc_results') or self.mu_visc_results is None:
            self._show_status("먼저 μ_visc 계산(Flash 활성)을 실행하세요.", 'warning')
            return
        ...
        if dT_profile is None or q_arr is None:
            self._show_status("배율별 ΔT 프로파일 데이터가 없습니다.", 'warning')
            return
        ...
        ttk.Label(dialog, text="배율별 ΔT 데이터를 CSV로 내보냅니다.\n"
                  "속도를 선택하세요 (여러 개 선택 가능).",
                  font=self.FONTS['body'], padding=10).pack(anchor=tk.W)

        # Velocity listbox with scrollbar
        ...
        v_listbox = tk.Listbox(list_frame, selectmode=tk.EXTENDED,
                               yscrollcommand=scrollbar.set, font=self.FONTS['mono_small'])
        v_listbox.pack(fill=tk.BOTH, expand=True)
        scrollbar.config(command=v_listbox.yview)

        # Populate with velocities; pre-select closest to 0.6758
        v_target = 0.6758
        j_closest = int(np.argmin(np.abs(v - v_target)))
        for j_idx, v_val in enumerate(v):
            dT_total = flash_results['delta_T'][j_idx]
            v_listbox.insert(tk.END, f"v={v_val:.4f} m/s  (ΔT={dT_total:.2f}°C)")
        v_listbox.selection_set(j_closest)
        v_listbox.see(j_closest)

        # All velocities checkbox
        all_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(dialog, text="전체 속도 내보내기 (하나의 CSV)", variable=all_var).pack(
        ...
        def do_export():
            ...
            if all_var.get():
                selected_indices = list(range(len(v)))
            else:
                selected_indices = list(v_listbox.curselection())
            if not selected_indices:
                self._show_status("속도를 선택하세요.", 'warning')
                return
            ...
            try:
                mc_prefix = self._get_mc_prefix()
                q0 = q_arr[0]
                log_zeta = np.log10(np.maximum(q_arr / q0, 1e-30))

                lines = [
                    f"# 배율별 Flash Temperature 프로파일",
                    f"# 생성일시,{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                    f"# 마스터커브,{mc_prefix if mc_prefix else 'N/A'}",
                    f"# T_base(°C),{T_base:.1f}",
                    f"# q₀(1/m),{q0:.6e}",
                    f"# 방식,Self-Consistent Iteration (Persson 2006)",
                    "#",
                ]

                # Header row
                header_parts = ["q(1/m)", "log10(zeta)"]
                for j_idx in selected_indices:
                    header_parts.append(f"T(C)_v={v[j_idx]:.4f}")
                    header_parts.append(f"dT(C)_v={v[j_idx]:.4f}")
                lines.append(",".join(header_parts))

                # Data rows
                for i in range(len(q_arr)):
                    row_parts = [f"{q_arr[i]:.6e}", f"{log_zeta[i]:.4f}"]
                    for j_idx in selected_indices:
                        T_val = T_base + dT_profile[i, j_idx]
                        dT_val = dT_profile[i, j_idx]
                        row_parts.append(f"{T_val:.4f}")
                        row_parts.append(f"{dT_val:.4f}")
                ...
                self._show_status(
                    f"배율별 ΔT 내보내기 완료\n{n_sel}개 속도, {len(q_arr)}개 파수\n{save_path}",
                    'success')
                self.status_var.set(f"배율별 ΔT CSV 저장 완료: {save_path}")
    ...
    def _create_mu_visc_tab(self, parent):
        ...
        # delnn: integration step size in log10(q) for flash temperature per-q loop
        ...
        try:
            ...
            if use_flash:
                ...
                if not has_aT:
                    ...
                    # h_layer = surface layer thickness, typically 1/q1
                    flash_h_layer = 1.0 / q[-1]  # Auto from upper cutoff wavenumber
                    ...
                    n_q = len(q)
                    delta_T_profile = np.zeros((n_q, n_v))
                    ...
                    # Create master curve E' and E'' log-log interpolators (boundary clamped)
                    ...
                    E_loss_master_interp_func = interp1d_flash(
                        log_omega_master, log_E_loss_master,
                        kind='linear', bounds_error=False,
                        fill_value=(log_E_loss_master[0], log_E_loss_master[-1])
                    )
                    E_storage_master_interp_func = interp1d_flash(
                        log_omega_master, log_E_storage_master,
                        kind='linear', bounds_error=False,
                        fill_value=(log_E_storage_master[0], log_E_storage_master[-1])
                    )

                    # Precompute angle array for inner integral
                    phi_flash = np.linspace(0, np.pi / 2, n_phi)
                    cos_phi_flash = np.cos(phi_flash)
                    prefactor_flash = 1.0 / ((1 - poisson**2) * sigma_0)

                    # Read SC iteration parameters from GUI (kept for GUI compatibility)
                    sc_max_iter = int(self.flash_max_iter_var.get())
                    sc_tol = float(self.flash_tol_var.get())
                    sc_relax = float(self.flash_relax_var.get())

                    # P_macro_cold: 핫스팟 면적비 (Phase 1에서 찾은 q_macro 위치)
                    # q_macro_auto가 설정되지 않았으면 q[0] 사용
                    q_macro_eff = q_macro_auto if q_macro_auto is not None else q[0]
                    i_macro = int(np.argmin(np.abs(q - q_macro_eff)))

                    log_aT_base = float(self.persson_aT_interp(temperature))
                    aT_base = 10**log_aT_base
                    print(f"\n[Flash] T_base={temperature}°C, log_aT_base={log_aT_base:.3f}")
                    print(f"[Flash] d_macro={flash_calc.d_macro*1e3:.3f} mm, "
                          f"h_layer={flash_calc.h_layer:.2e} m (1/q1), "
                          f"D_th={flash_calc.D_th:.2e} m²/s")
                    print(f"[Flash] q range: {q[0]:.2e} ~ {q[-1]:.2e} 1/m ({n_q} pts)")
                    print(f"[Flash] Method: Self-consistent iteration (Greenwood) / Per-q (Persson Full)")
                    ...
                    # Read Amax/Amin for per-velocity heat flux clamping (always available)
                    try:
                    ...
                    # Per-velocity Amax/Amin clamping applied to P for heat flux
                    for j in range(n_v):
                        P_for_cold = np.clip(A_A0_cold_arr[j], Amin_flash, Amax_flash)
                        q_dot_cold = flash_calc.heat_flux(
                            P_for_cold, mu_array_raw[j], sigma_0, v[j])
                        Jd_cold = flash_calc.peclet_number(v[j])
                        Jh_cold = flash_calc.peclet_number_h(v[j])
                        flash_delta_T_cold[j] = flash_calc.delta_T(q_dot_cold, Jd_cold, Jh_cold)

                    # Read flash model selection
                    flash_model_type = self.flash_model_var.get()  # "greenwood" or "persson_full"

                    # --- Fine q grid for per-q integration (delnn control) ---
                    try:
                    ...
                    log_q_max_hot = np.log10(q[-1])
                    n_q_hot = max(int(np.ceil((log_q_max_hot - log_q_min_hot) / delnn)) + 1, n_q)
                    q_hot = np.logspace(log_q_min_hot, log_q_max_hot, n_q_hot)

                    # Interpolate C(q) to fine grid (log-log)
                    log_C_q_interp_fine = interp1d_flash(
                        np.log10(q), np.log10(np.maximum(C_q, 1e-50)),
                        kind='linear', bounds_error=False, fill_value='extrapolate')
                    C_q_hot = 10.0 ** log_C_q_interp_fine(np.log10(q_hot))

                    print(f"[Flash] delnn={delnn}, q grid: {n_q} pts → {n_q_hot} pts (fine grid)")

                    def _hot_pass_uniform_T(v_j, T_hot_uniform, C_q_arr, strain_arr_j_local):
                        """Compute mu_hot and A/A0_hot at a single uniform temperature.
                        ...
                        S_hot = np.ones(n_local)
                        integrand_arr = np.zeros(n_local)
                        ...
                        for i in range(n_local):
                            ...
                            if n_phi >= 3:
                                ...
                                G_angle_integral_i = 4.0 * np.trapezoid(G_integrand_phi, phi_flash)

                            G_full_integrand_i = q_local[i]**3 * C_q_arr[i] * G_angle_integral_i

                            # Cumulative G via trapezoidal
                            if i > 0:
                                dq = q_local[i] - q_local[i - 1]
                                delta_G = 0.5 * (prev_G_integrand + G_full_integrand_i) * dq
                                G_hot[i] = G_hot[i - 1] + delta_G / (8.0 * norm_factor_val)
                            prev_G_integrand = G_full_integrand_i

                            # P(q) = erf(1 / (2*sqrt(G)))
                            if G_hot[i] > 1e-10:
                                arg = min(1.0 / (2.0 * np.sqrt(G_hot[i])), 10.0)
                                P_hot[i] = float(erf_flash(arg))
                            else:
                                P_hot[i] = 1.0
                            ...
                            if n_phi >= 3:
                                ...
                                mu_angle_integral_i = 4.0 * np.trapezoid(mu_phi_integrand, phi_flash)

                            integrand_arr[i] = q_local[i]**3 * C_q_arr[i] * P_hot[i] * S_hot[i] * mu_angle_integral_i

                        # Integrate mu over q
                        if n_local >= 3:
                            mu_hot = 0.5 * simpson_flash(integrand_arr, x=q_local)
                        else:
                    ...
                    def _hot_integration_v2(v_j, C_q_arr, strain_arr_j_local):
                        ...
                        Uses fine q grid (q_hot, n_q_hot) controlled by delnn parameter.
                        Returns results on the fine grid.
                        ...
                        S_hot = np.ones(n_local)
                        integrand_arr = np.zeros(n_local)
                        delta_T_per_q = np.zeros(n_local)
                        ...
                        for i in range(n_local):
                            ...
                            if flash_model_type == "persson_full":
                                ...
                                Jh_macro = flash_calc.peclet_number_h(v_j)
                                delta_T_q = flash_calc.delta_T(q_dot_macro, Jd_macro, Jh_macro)
                                T_acc = temperature + delta_T_q
                                delta_T_per_q[i] = T_acc - temperature

                            prev_mu_integrand = integrand_arr[i]

                        # Integrate μ over q (on fine grid)
                        if n_local >= 3:
                            mu_hot = 0.5 * simpson_flash(integrand_arr, x=q_local)
                        else:
                            mu_hot = 0.5 * np.trapezoid(integrand_arr, q_local)
                    ...
                    # --- Main Flash Temperature velocity loop ---
                    # Greenwood model: Self-consistent macroscopic iteration
                    #   All q evaluated at SAME T_hot; iterate mu↔ΔT until converged.
                    #   Prevents temperature runaway (U-shape in A/A0) at high v.
                    # Persson Full: Per-q forward pass with temperature accumulation.
                    model_name = ("Greenwood SC 반복" if flash_model_type == "greenwood"
                    ...
                    if flash_model_type == "greenwood":
                        print(f"[Flash] SC iteration: max_iter={sc_max_iter}, tol={sc_tol}°C, relax={sc_relax}")
                    print(f"[Flash] Amin={Amin_flash}, Amax={Amax_flash} (heat flux P clamping)")
                    ...
                    for j in range(n_v):
                        ...
                        # Interpolate C(q) to fine grid for this velocity
                        C_q_j_fine = C_q_hot  # Already interpolated from psd_model

                        # Strain array for nonlinear correction
                        strain_arr_j_fine = None
                        if strain_est is not None and use_fg:
                            strain_arr_j_coarse = strain_est(q, G_matrix_corrected[:, j], v[j])
                            # Interpolate strain to fine grid
                            if strain_arr_j_coarse is not None and len(strain_arr_j_coarse) == len(q):
                                log_strain_fine_interp = interp1d_flash(
                                    np.log10(q),
                                    np.log10(np.maximum(strain_arr_j_coarse, 1e-15)),
                                    kind='linear', bounds_error=False, fill_value='extrapolate')
                                strain_arr_j_fine = 10.0 ** log_strain_fine_interp(np.log10(q_hot))
                                strain_arr_j_fine = np.clip(strain_arr_j_fine, 0.0, 1.0)

                        if flash_model_type == "greenwood":
                            # ===== Self-Consistent Macroscopic Iteration (Greenwood) =====
                            # All q points evaluated at the SAME temperature T_hot.
                            # Iterate: mu_hot(T) → ΔT(mu_hot) → T_hot until converged.
                            # This prevents temperature runaway at high velocities:
                            #   high ΔT → low mu_hot → lower ΔT → converges to moderate T.
                            Jd_macro = v[j] * flash_calc.d_macro / flash_calc.D_th
                            Jh_macro = flash_calc.peclet_number_h(v[j])

                            # Initial ΔT from cold pass (with Amax/Amin clamping)
                            P_eff_init = np.clip(A_A0_cold_arr[j], Amin_flash, Amax_flash)
                            q_dot_init = flash_calc.heat_flux(P_eff_init, mu_array_raw[j], sigma_0, v[j])
                            delta_T_current = flash_calc.delta_T(q_dot_init, Jd_macro, Jh_macro)

                            mu_hot_iter = mu_array_raw[j]
                            A_A0_hot_iter = A_A0_cold_arr[j]
                            converged = False
                            ...
                            for sc_iter in range(sc_max_iter):
                                ...
                                P_eff_hot = np.clip(A_A0_hot_iter, Amin_flash, Amax_flash)
                                q_dot_new = flash_calc.heat_flux(P_eff_hot, mu_hot_iter, sigma_0, v[j])
                                delta_T_new = flash_calc.delta_T(q_dot_new, Jd_macro, Jh_macro)

                                # Under-relaxation for stability
                                delta_T_next = sc_relax * delta_T_new + (1.0 - sc_relax) * delta_T_current

                                # Convergence check
                                if abs(delta_T_next - delta_T_current) < sc_tol:
                                    delta_T_current = delta_T_next
                                    converged = True
                                    break
                            ...
                            # Store results
                            total_dT = delta_T_current
                            mu_hot_array_raw[j] = mu_hot_iter
                            A_A0_hot_arr[j] = A_A0_hot_iter
                            flash_delta_T[j] = total_dT
                            flash_T_hot[j] = temperature + total_dT
                            P_eff_result = np.clip(A_A0_hot_iter, Amin_flash, Amax_flash)
                            ...
                            mu_hot_iter, G_hot_iter, P_hot_iter, S_hot_iter, A_A0_hot_iter, dT_profile_j_fine = \
                                _hot_integration_v2(v[j], C_q_j_fine, strain_arr_j_fine)

                            # Interpolate delta_T profile from fine grid back to original q grid
                            dT_interp_back = interp1d_flash(
                                np.log10(q_hot), dT_profile_j_fine,
                                kind='linear', bounds_error=False,
                                fill_value=(0.0, dT_profile_j_fine[-1]))
                            dT_profile_j = dT_interp_back(np.log10(q))

                            total_dT = dT_profile_j_fine[-1]
                            mu_hot_array_raw[j] = mu_hot_iter
                            A_A0_hot_arr[j] = A_A0_hot_iter
                            flash_delta_T[j] = total_dT
                            flash_T_hot[j] = temperature + total_dT
                            ...
                            flash_q_dot[j] = mu_hot_iter * sigma_0 * v[j] / max(P_eff_result, 1e-6)
                            flash_Jd[j] = flash_calc.peclet_number(v[j])
                            flash_iterations[j] = 1
                            delta_T_profile[:, j] = dT_profile_j

                        # Diagnostic output
                        if j % max(1, n_v // 5) == 0 or j == n_v - 1:
                            iter_info = f", iter={flash_iterations[j]}" if flash_model_type == "greenwood" else ""
                            print(f"  v={v[j]:.4f} m/s: ΔT={total_dT:.2f}°C, "
                                  f"μ_cold={mu_array_raw[j]:.4f} → μ_hot={mu_hot_iter:.4f}, "
                                  f"A/A0_hot={A_A0_hot_iter:.4f}{iter_info}")

                        if j % max(1, n_v // 10) == 0:
                            self.status_var.set(
                                f"Flash ({model_name}): {j+1}/{n_v} (ΔT={total_dT:.1f}°C, μ_hot={mu_hot_iter:.3f})")
                            self.root.update()

                    # --- Dedicated profile at v_ref = 0.6758 m/s (for comparison) ---
                    v_ref = 0.6758
                    j_nearest = int(np.argmin(np.abs(v - v_ref)))
                    delta_T_profile_ref = delta_T_profile[:, j_nearest].copy()
                    print(f"\n  [Ref v={v_ref:.4f} m/s] ΔT={flash_delta_T[j_nearest]:.2f}°C, "
                          f"μ_hot={mu_hot_array_raw[j_nearest]:.4f}, "
                          f"iter={flash_iterations[j_nearest]}")

                    # Build flash_results dict
                    flash_results = {
                        'delta_T': flash_delta_T,
                        'delta_T_cold': flash_delta_T_cold,
                        'delta_T_profile': delta_T_profile,
                        'delta_T_profile_ref': delta_T_profile_ref,
                        'v_ref': v_ref,
                        'q_array': q.copy(),
                        'T_hot': flash_T_hot,
                        'q_dot': flash_q_dot,
            ...
            if use_flash and flash_results is not None and mu_hot_array is not None:
                ...
                self.mu_result_text.insert(tk.END, f"    delnn={flash_results.get('delnn', 'N/A')}, "
                                                   f"q grid: {n_q}→{flash_results.get('n_q_hot', n_q)} pts\n")
                self.mu_result_text.insert(tk.END, f"    Amin={flash_results.get('Amin', 'N/A')}, "
                                                   f"Amax={flash_results.get('Amax', 'N/A')} (P clamp)\n")
                flash_model_disp = flash_results.get('flash_model', 'greenwood')
                if flash_model_disp == "greenwood":
                    max_iter_disp = int(np.max(flash_results.get('iterations', [1])))
                    self.mu_result_text.insert(tk.END, f"    모델: Greenwood SC 반복 (max iter={max_iter_disp})\n")
                else:
                    self.mu_result_text.insert(tk.END, f"    모델: Persson Full Integral (Per-Scale)\n")
    ...
    def _create_variables_tab(self, parent):
        ...
        add_separator()
        add_text('Flash Temperature 중간 변수 (Per-q Sequential Accumulation):', bold=True, pady=(8, 0))

        add_equation(
            r"$D_{th} = \frac{\kappa}{\rho \cdot C_v}, \quad"
            r" J_d = \frac{v \cdot d}{D_{th}}, \quad"
            r" J_h = \frac{v \cdot h}{D_{th}}, \quad"
            r" d(q) = \frac{2\pi}{q}$",
            fig_height=1.0)
        ...
        add_equation(
            r"$\delta T(q_i) = \frac{\dot{q}(q_i) \cdot d(q_i)}"
            r"{2\kappa\,\sqrt{1 + \frac{\pi}{16} J_d(q_i) + \frac{J_h^2}{4}}}"
            r", \quad \Delta T = \sum_i \delta T(q_i)"
        ...
        unit_data = [
            ...
            ('d(q)', '[m]', '특성 접촉 직경 = 2\u03c0/q'),
            ('J_d', '[무차원]', 'Peclet 수 (거시) = v\u00b7d/D_th'),
            ('J_h', '[무차원]', 'Peclet 수 (표면층) = v\u00b7h/D_th, h=1/q\u2081'),
            ('\u0394T(q)', '[\u00b0C]', '파수별 누적 온도 상승'),
            ('a_T', '[무차원]', 'WLF 시프트 팩터'),
            ('q\u0307', '[W/m\u00b2]', '열유속 = d\u03bc\u00b7\u03c3\u2080\u00b7v'),
        ]
